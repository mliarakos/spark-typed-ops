package com.github.mliarakos.spark.sql.typed

import com.github.mliarakos.spark.sql.typed.ProjectionSpec._
import com.github.mliarakos.spark.sql.typed.ops._
import com.holdenkarau.spark.testing.DatasetSuiteBase
import org.apache.spark.sql.Dataset
import org.scalatest.flatspec.AnyFlatSpec
import org.scalatest.matchers.should.Matchers
import shapeless.test.illTyped

import scala.collection.immutable._
import scala.reflect.ClassTag

class ProjectionSpec extends AnyFlatSpec with Matchers with SparkMatchers with DatasetSuiteBase {
  import spark.implicits._

  private val dataset = Seq(
    Big("1", "John", 18, Seq("dog", "cat")),
    Big("2", "Sally", 23, Seq("dog"))
  ).toDS()

  private def validate[A: ClassTag](ds: Dataset[A], expected: Dataset[A]): Unit = {
    ds.columns shouldBe expected.columns
    ds shouldEqual expected
  }

  it should "project to itself" in {
    val big = dataset.project[Big]

    big shouldNot useObjectSerialization
    validate(big, dataset.map(record => record))
    validate(big, dataset.select("*").as[Big])
  }

  it should "project the same columns in a different order" in {
    val reorder = dataset.project[Reorder]

    reorder shouldNot useObjectSerialization
    validate(reorder, dataset.map(record => Reorder(record.age, record.pets, record.name, record.id)))
    validate(reorder, dataset.select("age", "pets", "name", "id").as[Reorder])
  }

  it should "project to fewer columns in the same order" in {
    val small = dataset.project[Small]

    small shouldNot useObjectSerialization
    validate(small, dataset.map(record => Small(record.name, record.age)))
    validate(small, dataset.select("name", "age").as[Small])
  }

  it should "project the fewer columns in a different order" in {
    val smallReorder = dataset.project[SmallReorder]

    smallReorder shouldNot useObjectSerialization
    validate(smallReorder, dataset.map(record => SmallReorder(record.age, record.name)))
    validate(smallReorder, dataset.select("age", "name").as[SmallReorder])
  }

  it should "not project invalid columns" in {
    illTyped { "ds.project[Wrong]" }
  }

}

object ProjectionSpec {
  final case class Big(id: String, name: String, age: Int, pets: Seq[String])
  final case class Reorder(age: Int, pets: Seq[String], name: String, id: String)
  final case class Small(name: String, age: Int)
  final case class SmallReorder(age: Int, name: String)
  final case class Wrong(recordId: String, name: String, age: Int)
}
