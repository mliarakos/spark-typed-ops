...
class MacroUsageSpec extends AnyFlatSpec with Matchers with SparkMatchers with DatasetSuiteBase with MacroTestFixtures with MacroTestData {
  ...
  it should "rename a column for a select" in {
    val outputs = inputs.select(renameFrom[Input, Output](_.id, _.id), renameFrom[Input, Output](_.start_date, _.startDate))
    validate(outputs, inputs.select(col("id").as("id"), col("start_date").as("startDate")))
  }
  ...
  it should "rename a column on a dataset for a select" in {
    val outputs = inputs.select(inputs.renameFrom[Output](_.id, _.id), inputs.renameFrom[Output](_.start_date, _.startDate))
    validate(outputs, inputs.select(inputs("id").as("id"), inputs("start_date").as("startDate")))
  }
